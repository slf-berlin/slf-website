...
| `/buero/ueber-uns` | `src/pages/Buero.jsx` (même composant) |
| `/buero/leistungen` | `src/pages/Buero.jsx` (même composant) |
| `/buero/team` | `src/pages/Buero.jsx` (même composant) |
| `*` | Redirect → `/` |

Pages not yet built: `/kontakt`.

### Büro page (`src/pages/Buero.jsx`)

Single-page layout, three numbered sections:
...
- **03 / Team** — `TEAM` array (7 members), split into `partners` (isPartner: true) and `mitarbeiter`. Each rendered by `<TeamMember>` sub-component: portrait photo (WP CDN) + name/role/education/CV timeline/Aufgabenfelder. Photos served directly from `https://www.slf-berlin.de/wordpress/wp-content/uploads/`.

Team member photo layout: flexbox row (column on mobile), photo `width: 200px` desktop / `120px` mobile, `aspectRatio: '3 / 4'`, `objectPosition: 'top'`.
...
**`project/design-canvas.jsx`** — standalone Figma-style design canvas, used for mocking up page variants before implementing them. Drop it into any HTML file that loads React + ReactDOM as globals.
...
- **Focus mode** — expand button or click label → fullscreen overlay; ←/→ navigates within section, ↑/↓ across sections, Esc exits
- **Export** — PNG (3×) or standalone HTML per artboard (kebab menu)
- **Persistence** — viewport (pan/zoom) → `localStorage`; artboard order/labels/hidden → `.design-canvas.state.json` sidecar via `window.omelette.writeFile`

Exports: `window.{DesignCanvas, DCSection, DCArtboard, DCPostIt}`.

## Suggested next steps

1. **Kontakt page** — form + map
2. **SEO** — per-page `<title>` / `<meta>` (react-helmet or Vite plugin)
3. **Responsive** — currently optimized for ≥ 768px desktop
