...
- Nav items and dropdown children defined in the `navItems` array at the top of `Nav.jsx`. Projekte dropdown items generated from `PROJEKTE_NAV` in `src/data/filters.js`. Büro dropdown: "Arbeitsweise" and "Leistungen" link to `/buero#arbeitsweise` / `/buero#leistungen` (hash anchors, handled by `ScrollToTop`)
...
The hero image (`deckblatt-homepage-v4.jpg`, 2831×1423) is a composite JPG of **4 sub-images** with white gaps. Seven absolutely-positioned flex segments (4 hotspots + 3 gap spacers) map to `LEISTUNGEN[0–3]`. `hoveredLeistung` state drives overlay opacity. Each hotspot is a `<Link>` to `/projekte?filter=<filterKey>` (`filterKey` field on each `LEISTUNGEN` entry; the hover reveal ends with a "Projekte ansehen →" line). Mapping: Stadtplanung → `stadtentwicklung`, Städtebau → `staedtebau`, Bauleitplanung → `bauleitplanung`, Verfahrensbetreuung → `verfahrensbetreuung`.
